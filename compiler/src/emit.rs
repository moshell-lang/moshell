...
pub fn emit(
    ...
    let from = instructions.current_ip();
    match &expr.kind {
        ExprKind::Declare(d) => {
            emit_declaration(d, instructions, ctx, cp, locals, state);

            let var = ctx.environment.variables.get_var(d.identifier).unwrap();
            if var.ty == TypeInfo::Function {
                // if it's a function declaration, then do not bind any position with start position
                // (no bytecode were emitted anyway)
                return;
            }
        }
        ExprKind::Block(exprs) => {
            emit_block(exprs, instructions, ctx, cp, locals, state);
            // do not bind generated bytecode with blocks' start positions
            return;
        }
    ...
    instructions.push_position(expr.segment.start, from)
}
