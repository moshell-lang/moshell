...
pub fn compile(typed_engine: &TypedEngine, engine: &Engine, typing: &Typing, writer: &mut impl Write) -> Result<(), io::Error> {
    ...
    for (id, chunk) in typed_engine.iter_chunks() {
        ...
        let signature = FunctionSignature::make(chunk_fqn.simple_name(), params, chunk.return_type, typing, &mut cp);
        bytecode.emit_constant_ref(cp.insert_signature(signature));
        compile_instruction_set(once(&chunk.expression), &mut bytecode, typing, &mut cp);
    ...
    bytecode.emit_constant_ref(cp.insert_signature(signature));
    compile_instruction_set(scripts.iter().map(|c| &c.expression), &mut bytecode, typing, &mut cp);
...
fn compile_instruction_set<'a>(expressions: impl Iterator<Item=&'a TypedExpr>,
    ...
    for expr in expressions {
        emit(expr, &mut instructions, typing, cp, &mut EmissionState::default());
    }
